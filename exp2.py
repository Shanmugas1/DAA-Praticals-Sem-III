        ...
        if p_hash == t_hash:
            for k in range(m):           
                ...
                if text[s+k]!=pattern[k]:
                    ...
                    matches.append(s)
        if s<n-m:
            t_hash= (d*(t_hash - ord(text[s]))+ord(text[s+m]))%q
            if t_hash<0:
                t_hash+=q
        return matches, comparisons
text = 'AABBAACAADAABAABA'
pattern = 'AABA'
print(f'Text: {text}')
print(f'Pattern: {pattern}')
m1,c1 = naive_search(text,pattern)
m2,m3 = kpm_search(text,pattern)
m3,c3 = rabin_karp(text,pattern)

print(f'\nNaive -> Matches at: {m1}, Comparison: {c1}')
print(f'KMP -> Matches at: {m1}, Comparison: {c1}')
print(f'RK -> Matches at: {m1}, Comparison: {c1}')

text_large = ''.join(random.choice('ABCD',k=10000))
